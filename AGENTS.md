...
- Canonical suite: `.venv/bin/python tests.py` (219 tests). Prefer the venv interpreter pinned by `.cursor/install.sh` (CPython 3.14.5).
...
- `sibling_spike_enabled=True` (default): quote watch-only opposite YES contracts and **block entry** if a sibling mid rises by `sibling_spike_cents` (default 15) within `sibling_spike_lookback_s` (default 45s). Catches underdog rips (e.g. 5c→40c) the score model misses.
