...
- Canonical suite: `.venv/bin/python tests.py` (212 tests). Prefer the venv interpreter pinned by `.cursor/install.sh` (CPython 3.14.5).
...
- Paper runtime attaches an **ESPN ATP/WTA score + win-prob gate** (`espn_prob_gate.py`). Entries require a name-bind to a live/pre ESPN match and model edge vs ask. Unbound ITF markets fail closed (no dip-only buys). Sportradar is not used. Toggle: `Config.espn_gate_enabled`.
